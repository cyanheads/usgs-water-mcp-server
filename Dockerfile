...
# Install only runtime dependencies. Direct dependencies such as DuckDB remain;
# framework optional peer tiers are omitted unless explicitly installed below.
RUN --mount=type=cache,target=/root/.bun/install/cache \
...
RUN mkdir -p /var/log/usgs-water-mcp-server && chown -R bun:bun /var/log/usgs-water-mcp-server
...
ENV LOGS_DIR="/var/log/usgs-water-mcp-server"
